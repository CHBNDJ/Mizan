...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signUp = async (
    ...
    try {
      // ── Metadata pour auth.users (lu par le trigger Supabase) ──
      const metaData: any = {
        firstName: userData.firstName,
        lastName: userData.lastName,
        phone: userData.phone || null,
      ...
      // ── Attente trigger Supabase (crée les lignes users + lawyers) ──
      await new Promise((resolve) => setTimeout(resolve, 2500));

      // ── Mise à jour directe : champs que le trigger peut rater ────
      try {
        // 1. Langues dans users (souvent absent du trigger)
        ...
        // 2. Adresse dans users (pour les professionnels)
        ...
        // 3. Profession dans lawyers (champ critique pour multi-profession)
        ...
        // Non bloquant — on continue l'inscription même si la mise à jour échoue
