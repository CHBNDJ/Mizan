          ...
          <>
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 mb-4 flex items-start gap-2.5">
              ...
              <p className="text-xs text-blue-700 leading-relaxed">
                ...
                message. Le paiement sera convenu directement.
              </p>
            </div>
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
  ...
  return (
    <div className="min-h-screen pt-16 bg-gradient-to-br from-teal-100 via-white to-teal-100 overflow-x-hidden w-full">
      <div className="max-w-4xl mx-auto px-4 py-8">
                ...
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  ...
                  {(!user || isClient) && !isOwnProfile && (
                    <div className="flex items-center gap-2 flex-wrap">
                      <Link href={`/consultation/${avocat.id}`}>
                        <button className="cursor-pointer bg-teal-600 hover:bg-teal-700 text-white py-2.5 px-4 rounded-xl flex items-center gap-2 font-semibold text-sm transition-all shadow-sm">
                          <MessageCircle className="w-4 h-4" /> Consulter
                        </button>
                      </Link>
                      {isAppointmentProfession && (
                        <button
                          ...
                          className="cursor-pointer bg-white border border-teal-300 text-teal-700 hover:bg-teal-50 py-2.5 px-4 rounded-xl flex items-center gap-2 font-semibold text-sm transition-all shadow-sm"
                        >
                          <Calendar className="w-4 h-4" /> RDV physique
                        </button>
                      )}
                      <button
                        onClick={handleVideoCall}
        ...
        {pricingChannels && pricingChannels.length > 0 && (
          <div className="content-card opacity-0 invisible mb-4 bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            ...
            <div className="divide-y divide-slate-100">
              {pricingChannels.map((ch: any) => {
                const icons: Record<string, any> = {
                  message: <MessageCircle className="w-4 h-4 text-teal-600" />,
                  phone: <Phone className="w-4 h-4 text-teal-600" />,
                  video_30: <Video className="w-4 h-4 text-teal-600" />,
                  video_60: <Video className="w-4 h-4 text-teal-600" />,
                  email: <Mail className="w-4 h-4 text-teal-600" />,
                };
                return (
                  <div
                    key={ch.type}
                  ...
                  >
                    ...
                    <span className="text-sm font-semibold text-teal-700">
                      {ch.base_price
                        ? `À partir de ${ch.base_price.toLocaleString()} DA`
                        : "Tarif sur demande"}
                    </span>
                  </div>
                );
        ...
        {isClient && !isOwnProfile && (
          <div className="content-card opacity-0 invisible lg:hidden mb-4 flex flex-col gap-3">
            <Link href={`/consultation/${avocat.id}`} className="block">
              <button className="cursor-pointer w-full bg-teal-600 hover:bg-teal-700 text-white py-3 px-4 rounded-xl flex items-center justify-center gap-2 font-semibold text-sm shadow-sm">
                <MessageCircle className="w-4 h-4" /> Consulter
              </button>
            </Link>
            {isAppointmentProfession && (
              <button
                ...
                className="cursor-pointer w-full bg-white border border-teal-300 text-teal-700 hover:bg-teal-50 py-3 px-4 rounded-xl flex items-center justify-center gap-2 font-semibold text-sm shadow-sm"
              >
                <Calendar className="w-4 h-4" /> RDV physique
              </button>
            )}
