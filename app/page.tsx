...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen pt-16 bg-gradient-to-br from-teal-100 via-white to-teal-100 overflow-x-hidden">
      ...
      {/* Pourquoi choisir — liens vers pages explicatives professions */}
      <section className="px-4 pb-10">
        <div className="max-w-5xl mx-auto">
          <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6 shadow-sm">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-4 text-center">
              Comprendre chaque profession
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {PROFESSIONS.map(({ id, label, Icon }) => (
                <Link key={id} href={`/professions/${id}`}>
                  <div className="flex items-center justify-between gap-3 p-3.5 border border-slate-100 hover:border-teal-200 rounded-xl cursor-pointer transition-all hover:bg-teal-50/30 group">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <span className="text-sm font-medium text-slate-700 truncate">
                        Pourquoi un {label.toLowerCase()} ?
                      </span>
                    </div>
                    <span className="text-xs text-teal-600 font-medium group-hover:text-teal-700 flex-shrink-0">
                      En savoir plus →
                    </span>
                  </div>
                </Link>
