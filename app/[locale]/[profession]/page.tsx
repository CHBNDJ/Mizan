...
function ProfessionContent({ profId }: { profId: ProfId }) {
  ...
  const langueOptions = TRADUCTEUR_LANGUES.map((l) => ({
    value: l,
    label: getLangueLabel(l, t),
  }));
  ...
  const handleSearch = () => {
    ...
    selectedLangues.forEach((l) => p.append("langues", l));
              ...
              >
                ...
                {profId === "traducteur" && (
                  <div className="relative" style={{ zIndex: 15 }}>
                    <label className="block text-xs font-semibold text-slate-500 dark:text-[#A8A8A6] uppercase tracking-wide mb-2">
                      {t("professionLanding.languesLabel")}
                    </label>
                    <MultiSelectWithCheckboxes
                      placeholder={t("professionLanding.languesPlaceholder")}
