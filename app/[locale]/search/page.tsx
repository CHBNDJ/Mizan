...
function SearchResults() {
  ...
  const readFilters = (): SearchFilters => {
    ...
    if (langs.length) f.langues = langs;
    return f;
  };
  ...
  const updateURL = (f: SearchFilters) => {
    ...
    if (f.langues) {
      const arr = Array.isArray(f.langues) ? f.langues : [f.langues];
      arr.forEach((l) => p.append("langues", l));
    }
  ...
  const hasLightFilters = !!(
    ...
    (Array.isArray(filters.langues) ? filters.langues.length : filters.langues)
  );
  ...
  const LANGUAGE_OPTIONS_TRADUCTEUR = [
    ...LANGUAGE_OPTIONS_BASE,
    {
      value: "Allemand",
      label:
        locale === "ar" ? "الألمانية" : locale === "en" ? "German" : "Allemand",
    },
    {
      value: "Russe",
      label:
        locale === "ar" ? "الروسية" : locale === "en" ? "Russian" : "Russe",
    },
    {
      value: "Chinois",
      label:
        locale === "ar" ? "الصينية" : locale === "en" ? "Chinese" : "Chinois",
    },
    {
      value: "Italien",
      label:
        locale === "ar" ? "الإيطالية" : locale === "en" ? "Italian" : "Italien",
    },
    {
      value: "Turc",
      label: locale === "ar" ? "التركية" : locale === "en" ? "Turkish" : "Turc",
    },
  ];

  const LANGUAGE_OPTIONS =
    professionParam === "traducteur"
      ? LANGUAGE_OPTIONS_TRADUCTEUR
      : LANGUAGE_OPTIONS_BASE;

  const selectedLangues = Array.isArray(filters.langues)
    ? filters.langues
    : filters.langues
      ? [filters.langues]
      : [];
