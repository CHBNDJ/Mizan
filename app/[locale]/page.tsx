...
function ProfCard({ id, Icon, label, desc, size = "normal" }: any) {
  return (
    <Link href={`/${id}`}>
      <div
        className={`prof-card prof-card-btn rounded-2xl border-2 cursor-pointer h-full flex flex-col items-center text-center transition-all
          bg-white/15 backdrop-blur-sm border-white/25 hover:bg-white/25 hover:border-white/50
          ${size === "big" ? "px-6 py-6 gap-3" : "px-4 py-4 gap-2"}`}
      >
        <div
          className={`flex-shrink-0 rounded-xl bg-white/20 border border-white/30 flex items-center justify-center
            ${size === "big" ? "w-12 h-12" : "w-9 h-9"}`}
        >
          <Icon
            className={`text-white ${size === "big" ? "w-6 h-6" : "w-4 h-4"}`}
          />
        </div>
        <div>
          <div
            className={`font-bold text-white ${size === "big" ? "text-base" : "text-sm"}`}
          >
            {label}
          </div>
          <div className="text-xs text-white/70 mt-0.5 leading-relaxed line-clamp-2">
            {desc}
          </div>
        </div>
      </div>
...
function ProfCardHorizontal({ id, Icon, label, desc }: any) {
  return (
    <Link href={`/${id}`}>
      <div
        className="prof-card prof-card-btn rounded-2xl border-2 cursor-pointer flex items-center gap-4 px-4 py-4 transition-all
        bg-white/15 backdrop-blur-sm border-white/25 hover:bg-white/25 hover:border-white/50"
      >
        <div className="w-10 h-10 flex-shrink-0 rounded-xl bg-white/20 border border-white/30 flex items-center justify-center">
...
export default function HomePage() {
  ...
  const heroTitle = (
    <>
      {t("home.hero.title1")}
      <br className="hidden sm:block" />{" "}
      <span className="text-teal-300">{t("home.hero.title2")}</span>
    </>
  );
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#141415] overflow-x-hidden">
      <style>{`
        .hero-badge,.hero-title,.hero-sub,.prof-card,.stat-card,.steps-section,.avocat-card,.cta-section,.prof-roles-link,.testimonials-section,.feedback-cta { opacity:0; }
        .prof-card-btn { transition:all 0.2s ease; }
        .prof-card-btn:hover { transform:translateY(-3px); }
        .hero-video-overlay {
          background: linear-gradient(
            to bottom,
            rgba(4,52,44,0.55) 0%,
            rgba(15,110,86,0.45) 40%,
            rgba(4,52,44,0.75) 100%
          );
        }
      `}</style>

      {/* ── HERO avec vidéo ── */}
      <section className="relative min-h-[100svh] sm:min-h-screen flex flex-col justify-center overflow-hidden pt-16">
        {/* Vidéo background */}
        <video
          ref={videoRef}
          autoPlay
          ...
          style={{ zIndex: 0 }}
        >
          {/* Vidéo Pexels — cabinet d'avocats / salle d'audience */}
          <source
            src="https://videos.pexels.com/video-files/5668859/5668859-uhd_2560_1440_25fps.mp4"
            type="video/mp4"
          />
          {/* Fallback si la vidéo ne charge pas */}
          <source
            src="https://videos.pexels.com/video-files/3044953/3044953-uhd_2560_1440_25fps.mp4"
            type="video/mp4"
          />
        </video>

        {/* Overlay teal dégradé */}
        <div
          className="hero-video-overlay absolute inset-0"
          style={{ zIndex: 1 }}
        />

        {/* Fallback image si vidéo indisponible */}
        <div
        ...
        {/* Contenu */}
        <div className="relative px-4 py-14 sm:py-20" style={{ zIndex: 2 }}>
          <div className="max-w-4xl mx-auto text-center">
            {/* Badge */}
            <div className="hero-badge inline-flex items-center gap-2 bg-white/15 backdrop-blur-sm border border-white/25 rounded-full px-4 py-1.5 mb-6">
              <span className="w-2 h-2 rounded-full bg-teal-300 animate-pulse" />
              <span className="text-xs font-medium text-white/90">
                {t("home.hero.badge", {
                  defaultValue: "Plateforme juridique algérienne",
                })}
              </span>
            </div>

            <h1 className="hero-title text-3xl sm:text-5xl lg:text-6xl font-bold text-white mb-5 leading-tight tracking-tight">
              {heroTitle}
            </h1>
            <p className="hero-sub text-sm sm:text-lg text-white/80 mb-10 sm:mb-14 max-w-xl mx-auto leading-relaxed">
              {t("home.hero.sub1")}
              <br />
              {t("home.hero.sub2")}
            </p>

            {/* Cartes professions — mobile */}
            <div className="flex flex-col gap-3 max-w-sm mx-auto sm:hidden">
              {PROFESSIONS.map((p) => (
                <ProfCardHorizontal key={p.id} {...p} />
              ))}
        ...
        >
          <span className="text-[10px] text-white/50 font-medium uppercase tracking-widest">
            {t("home.hero.scroll", { defaultValue: "Découvrir" })}
          </span>
      ...
      {/* ── Reste de la page — fond normal ── */}
      <div className="bg-gradient-to-br from-teal-50 via-white to-teal-50 dark:bg-none">
        <section className="px-4 pb-4 pt-8">
          <div className="max-w-5xl mx-auto text-center">
            <Link href="/professions" className="prof-roles-link inline-block">
              <span className="text-xs font-medium text-teal-600 dark:text-[#6fcf9f] hover:text-teal-700 cursor-pointer transition-colors">
                {t("home.profLink")}
              </span>
            </Link>
          </div>
          ...
          <section className="avocats-section pb-16 sm:pb-20 px-4">
            <div className="max-w-6xl mx-auto">
              ...
              <div className="text-center">
                <button
                  onClick={() => router.push("/search")}
                  className="text-teal-600 dark:text-[#6fcf9f] inline-flex items-center gap-1.5 hover:text-teal-700 cursor-pointer text-sm font-medium"
                >
                  {t("home.topRated.seeAll")} <ArrowRight className="w-4 h-4" />
                </button>
              </div>
