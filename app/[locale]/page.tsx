...
function ProfCard({ id, Icon, label, desc, size = "normal" }: any) {
  return (
    <Link href={`/${id}`}>
      <div
        className={`prof-card prof-card-btn bg-white dark:bg-[#1c1c1e] rounded-2xl border-2 border-slate-200 dark:border-zinc-800 cursor-pointer h-full flex flex-col items-center text-center hover:border-teal-400 dark:hover:border-teal-500 hover:shadow-md transition-all ${size === "big" ? "px-6 py-6 gap-3" : "px-4 py-4 gap-2"}`}
      >
        <div
          className={`flex-shrink-0 rounded-xl bg-teal-50 dark:bg-[#2c2c2e] border border-teal-100 dark:border-zinc-700 flex items-center justify-center ${size === "big" ? "w-12 h-12" : "w-9 h-9"}`}
        >
          <Icon
            className={`text-teal-600 dark:text-teal-400 ${size === "big" ? "w-6 h-6" : "w-4 h-4"}`}
          />
        </div>
        <div>
          <div
            className={`font-bold text-slate-800 dark:text-zinc-100 ${size === "big" ? "text-base" : "text-sm"}`}
          >
            {label}
          </div>
          <div className="text-xs text-slate-400 dark:text-zinc-500 mt-0.5 leading-relaxed line-clamp-2">
            {desc}
          </div>
        </div>
      </div>
...
function ProfCardHorizontal({ id, Icon, label, desc }: any) {
  return (
    <Link href={`/${id}`}>
      <div className="prof-card prof-card-btn bg-white dark:bg-[#1c1c1e] rounded-2xl border-2 border-slate-200 dark:border-zinc-800 cursor-pointer flex items-center gap-4 px-4 py-4 hover:border-teal-400 dark:hover:border-teal-500 hover:shadow-md transition-all">
        <div className="w-10 h-10 flex-shrink-0 rounded-xl bg-teal-50 dark:bg-[#2c2c2e] border border-teal-100 dark:border-zinc-700 flex items-center justify-center">
        ...
        <div className="text-start">
          ...
          <div className="text-xs text-slate-400 dark:text-zinc-500 mt-0.5 leading-relaxed">
            {desc}
          </div>
        </div>
      </div>
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen pt-16 bg-gradient-to-br from-teal-100 via-white to-teal-100 dark:bg-gradient-to-tr dark:from-black dark:via-black dark:to-[#0a3d22] dark:bg-fixed overflow-x-hidden">
      <style>{`
        .hero-title,.hero-sub,.prof-card,.stat-card,.steps-section,.avocat-card,.cta-section,.prof-roles-link,.testimonials-section,.feedback-cta { opacity:0; }
        .prof-card-btn { transition:all 0.2s ease; }
        .prof-card-btn:hover { transform:translateY(-4px); box-shadow:0 16px 40px rgba(13,148,136,0.15); border-color:#0D9488 !important; }
      ...
      <div className="fixed top-20 end-4 z-50">
        <ThemeToggle />
      </div>

      <section className="py-14 sm:py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="hero-title text-2xl sm:text-4xl lg:text-6xl font-bold text-slate-800 dark:text-zinc-50 mb-5 leading-tight tracking-tight">
            {heroTitle}
          </h1>
          <p className="hero-sub text-sm sm:text-lg text-slate-500 dark:text-zinc-400 mb-10 sm:mb-14 max-w-xl mx-auto leading-relaxed">
            {t("home.hero.sub1")}
            <br />
            {t("home.hero.sub2")}
          </p>
      ...
      <section className="px-4 pb-14 sm:pb-20 pt-2">
        <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          {statsData.map((s) => (
            <div
              key={s.label}
              className="stat-card bg-white dark:bg-[#1c1c1e] rounded-2xl shadow-sm border border-slate-100 dark:border-zinc-800 p-4 md:p-6 flex flex-col items-center text-center"
            >
              <AnimatedCounter
                end={s.end}
                duration={2000}
                suffix={s.suffix}
                className="text-3xl sm:text-4xl font-bold text-teal-600 dark:text-teal-400 mb-2"
              />
              <div className="text-slate-600 dark:text-zinc-400 text-xs sm:text-sm leading-snug">
                {s.label}
              </div>
            </div>
          ))}
      ...
      <section className="steps-section py-12 sm:py-14 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="bg-white dark:bg-[#1c1c1e] rounded-2xl shadow-sm p-6 sm:p-10">
            <h2 className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-zinc-50 mb-6 md:mb-10 text-center">
              {t("home.howItWorks.title")}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
              {howItWorksSteps.map((s, i) => (
                <div
                  key={s.title}
                  className={`flex gap-4 ${i < 2 ? "md:border-e md:border-slate-100 dark:md:border-zinc-800 md:pe-10 pb-6 md:pb-0 border-b md:border-b-0 border-slate-100 dark:border-zinc-800" : ""}`}
                >
                  <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-teal-50 dark:bg-[#2c2c2e] border border-teal-200 dark:border-zinc-700 flex items-center justify-center text-sm font-bold text-teal-700 dark:text-teal-400 flex-shrink-0 mt-0.5">
                    {ld(String(i + 1))}
                  </div>
                  <div>
                    <div className="text-sm sm:text-base font-semibold text-slate-800 dark:text-zinc-100 mb-1.5">
                      {s.title}
                    </div>
      ...
      {topAvocats.length > 0 && (
        <section className="avocats-section pb-16 sm:pb-20 px-4">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-8 sm:mb-10">
              <h2 className="text-2xl sm:text-3xl font-bold text-slate-800 dark:text-zinc-50 mb-3">
                {t("home.topRated.title")}
              </h2>
            ...
            <div className="text-center">
              <button
                onClick={() => router.push("/search")}
                className="text-teal-600 dark:text-teal-400 inline-flex items-center gap-1.5 hover:text-teal-700 dark:hover:text-teal-300 cursor-pointer text-sm font-medium"
              >
                {t("home.topRated.seeAll")} <ArrowRight className="w-4 h-4" />
              </button>
            </div>
      ...
      <section className="cta-section py-12 sm:py-14 px-4">
        <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
          <div className="bg-white dark:bg-[#1c1c1e] rounded-2xl shadow-sm p-7 sm:p-10 flex flex-col text-center sm:text-start">
            <h3 className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-zinc-50 mb-3">
              {t("home.ctaClient.title")}
            </h3>
            <p className="text-slate-500 dark:text-zinc-400 leading-relaxed mb-8 text-sm sm:text-base">
              {t("home.ctaClient.desc")}
            </p>
            <div className="mt-auto flex justify-center sm:justify-start">
              <button
