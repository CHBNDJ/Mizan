...
//               }
//               className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 dark:text-[#7A7A78] hover:text-slate-600 dark:text-[#E8E8E6] cursor-pointer"
//             >
...
//                     className="text-xs text-slate-500 dark:text-[#A8A8A6] hover:text-slate-700 dark:text-[#E8E8E6] cursor-pointer px-2 py-1 rounded-lg hover:bg-slate-100"
//                   >
...
//               </div>
//             ) : confirmAction === "close" ? (
//               <div className="flex-1 bg-teal-50 dark:bg-[#6fcf9f]/10 border border-teal-200 dark:border-[#6fcf9f]/20 rounded-xl p-3 flex items-center justify-between gap-2">
...
//                     disabled={isActioning}
//                     className="text-xs font-semibold text-white bg-teal-600 dark:bg-[#0F6E56] hover:bg-teal-700 dark:hover:bg-[#085041] px-3 py-1 rounded-lg cursor-pointer disabled:opacity-50"
//                   >
//                     {isActioning ? "..." : t("lawyerConsultations.confirm")}
//                   </button>
//                 </div>
...
//                   className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-medium text-teal-700 dark:text-[#6fcf9f] bg-teal-50 dark:bg-[#6fcf9f]/10 hover:bg-teal-100 dark:hover:bg-[#26492f] border border-teal-200 dark:border-[#6fcf9f]/20 rounded-xl cursor-pointer transition-all"
//                 >
...
//         {isClosed && (
//           <div className="mt-3 flex items-center gap-2 bg-slate-50 border border-slate-200 dark:border-[#1c2220] rounded-xl px-3 py-2">
...
//           >
//             <div
//               className={`max-w-[75%] rounded-xl p-3.5 ${message.sender_type === "lawyer" ? "bg-teal-600 dark:bg-[#0F6E56] text-white" : "bg-slate-100 text-slate-900 dark:text-[#F5F5F4]"}`}
//             >
//               {message.message && (
...
//                 className="cursor-pointer px-3 border border-slate-200 dark:border-[#1c2220] rounded-lg hover:border-teal-400 dark:hover:border-[#6fcf9f] hover:bg-teal-50 dark:hover:bg-[#26492f]"
...
//                   uploading
//                 }
//                 className="cursor-pointer px-4 bg-teal-600 dark:bg-[#0F6E56] text-white rounded-lg hover:bg-teal-700 dark:hover:bg-[#085041] disabled:opacity-50"
//               >
//                 {isSending || uploading ? (
...
//                     className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all cursor-pointer ${tabFilter === tab.key ? "bg-teal-600 dark:bg-[#0F6E56] text-white" : "bg-white dark:bg-[#1c1c1e] border border-slate-200 dark:border-[#1c2220] text-slate-600 dark:text-[#E8E8E6] hover:bg-slate-50"}`}
//                   >
...
//                                 <span className="inline-flex items-center gap-1 text-[10px] font-medium text-slate-500 dark:text-[#A8A8A6] bg-slate-100 px-1.5 py-0.5 rounded-full">
...
function LawyerConsultationsContent() {
  ...
  const isClosed = selectedConsultation?.status === "closed";
  // Refuser n'a de sens que sur une demande encore "en attente" (message
  // simple). Une consultation auto-confirmée (téléphone/vidéo/physique) est
  // déjà "accepted" et ne peut plus être "refusée" — seul Clôturer garde un sens.
  const isPending = selectedConsultation?.status === "pending";
  ...
  // IMPORTANT : ChatPanel est appelé comme une fonction normale ChatPanel(),
  // jamais comme un composant <ChatPanel />. Le déclarer comme composant et
  // l'instancier via <ChatPanel /> recréait une "nouvelle identité" de
  // composant à chaque frappe dans le textarea (puisque newMessage change à
  // chaque lettre et re-render le parent), ce qui faisait perdre le focus du
  // champ de texte après chaque caractère tapé.
  const ChatPanel = () => (
    <div className="bg-white dark:bg-[#1c1c1e] rounded-xl shadow-sm dark:shadow-none border border-slate-200 dark:border-[#1c2220] flex flex-col h-[600px]">
      <div className="p-5 bg-gradient-to-r from-teal-50 to-white border-b border-slate-200 dark:border-[#1c2220]">
        <div className="flex items-start gap-3">
