...
function buildEmail(opts: {
  ...
  const ctaBlock = opts.isPhysical
    ? `
      ${
        opts.address
          ? `<p style="background:#f0f9f4; border-radius:8px; padding:12px 16px;">📍 ${opts.address}</p>`
          : ""
      }
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto; color: #1c1c1e;">
      ...
      <p>Votre ${opts.isPhysical ? "rendez-vous" : "consultation"} <strong>${opts.channelLabel}</strong> avec ${opts.otherName} ${opts.isPhysical ? "a lieu" : "commence"} <strong>${opts.minutesLabel}</strong>.</p>
...
async function runReminders(req: NextRequest) {
  ...
  try {
    const { data: consults, error } = await supabaseAdmin
      ...
      .in("channel", ["video_30", "video_60", "phone", "physical"])
      .neq("status", "closed");
    ...
    for (const c of consults || []) {
      ...
      const isPhysical = c.channel === "physical";
      const channelLabel = isPhysical
        ? "au cabinet"
        : c.channel === "phone"
          ? "téléphone"
          : "vidéo";
      ...
      const joinUrl = isPhysical
        ...
        : `${APP_URL}/consultation/${c.id}/video`;
      const lawyerJoinUrl = isPhysical
        ? `${APP_URL}/lawyer/consultations`
        : `${APP_URL}/consultation/${c.id}/video`;
