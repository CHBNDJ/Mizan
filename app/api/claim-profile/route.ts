...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. Vérifier le code depuis la DB
    ...
    // 6. Vérifier si compte existe déjà
    const { data: existingAuthData } = await supabase.auth.admin.listUsers();
    const existingAuthUser = existingAuthData?.users.find(
      (u) => u.email === email
    );
    ...
    // 8. Attendre que les triggers créent users + lawyers
    await new Promise((resolve) => setTimeout(resolve, 2000));

    // 9. ✅ UPDATE (pas DELETE/INSERT) le profil users
    ...
    // 10. ✅ UPDATE (pas DELETE/INSERT) le profil lawyers
    ...
    // 12. Marquer le code comme utilisé
    await supabase
      .from("claim_verification_codes")
      .update({ used: true })
      .eq("id", verification.id);

    // 13. Supprimer l'ancien profil
    try {
      await supabase.from("lawyers").delete().eq("id", lawyerId);
      await supabase.from("users").delete().eq("id", lawyerId);
    } catch (deleteError) {
