...
// ── SVG logo Mizan (balance Lucide) inline ────────────────────────────────────
const mizanLogoSvg = `
...
// ── Template email — design 2 (minimaliste, trait teal, logo SVG) ─────────────
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Ignorer si ce n'est pas une validation (false → true)
    if (!lawyerId || wasVerified || !isNowVerified) {
      return NextResponse.json({ message: "Ignoré" });
    }

    // Récupérer email + nom + profession + barreau
