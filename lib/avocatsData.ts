    ...
    titre:
      lawyer.profession === "notaire" || lawyer.profession === "huissier"
        ...
        : lawyer.profession === "comptable" ||
            lawyer.profession === "expert-comptable" ||
            lawyer.profession === "traducteur"
          ? "M."
          : "Maître",
...
export async function searchAvocats(
  ...
  return all.filter((avocat) => {
    ...
    if (filters.langues) {
      const wanted = Array.isArray(filters.langues)
        ? filters.langues
        : [filters.langues];
      if (wanted.length > 0) {
        const ok = wanted.some((w) =>
          avocat.langues?.some((l) => l.toLowerCase().includes(w.toLowerCase()))
        );
        if (!ok) return false;
      }
    }
...
export async function getAvocatById(
  ...
  try {
    ...
    const { data: lawyer } = await supabase
      .from("lawyers")
      .select(
        "id, slug, bar_number, specializations, experience_years, consultation_price, is_verified, is_claimed, claimed_at, rating_google, reviews_count_google, rating_mizan, reviews_count_mizan, available_now, available_now_since, updated_at, created_at, profession, professions, is_cour_supreme, is_assermente, bio"
      )
