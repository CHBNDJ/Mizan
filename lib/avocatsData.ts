...
//     notes:
//       "Base de données MVP Mizan - 21 avocats individuels 100% vérifiés dans 3 wilayas (Alger, Blida, Sétif). Affichage dynamique selon avocats inscrits et vérifiés. Statistiques calculées depuis public.lawyers + public.users.",
//   },
// };
...
export const AVOCATS_DATABASE: AvocatsDatabase = {
  ...
  metadata: {
    ...
    source: "Supabase - Données dynamiques basées sur avocats vérifiés",
    notes:
      "Base de données MVP Mizan - 21 avocats individuels 100% vérifiés dans 3 wilayas.",
  },
};
...
function convertSupabaseToAvocatData(lawyer: any): AvocatData {
  ...
  return {
    ...
    claimed_at: lawyer.claimed_at,
    consultation_price: lawyer.consultation_price,
    // Date d'inscription sur Mizan — utilisée pour le tri "Récemment inscrits"
...
export async function getSupabaseAvocats(): Promise<AvocatData[]> {
  ...
  try {
    ...
    const combinedData = lawyers
      .map((lawyer) => {
        const user = users?.find((u) => u.id === lawyer.id);
        if (user) return { ...lawyer, users: user };
        return null;
      })
      .filter((item) => item !== null);
...
export async function getWilayas(): Promise<string[]> {
  try {
    ...
    const wilayasSet = new Set<string>();
    allAvocats.forEach((avocat) => {
      if (avocat.wilaya) wilayasSet.add(avocat.wilaya);
    });
    return Array.from(wilayasSet).sort();
  } catch (error) {
    return [];
...
export async function getAvocatById(id: string): Promise<AvocatData | null> {
  ...
  try {
    const { data: lawyer, error: lawyerError } = await supabase
      ...
      .select(
        `
        id, bar_number, specializations, experience_years, consultation_price,
        is_verified, is_claimed, claimed_at, rating_google, reviews_count_google,
        rating_mizan, reviews_count_mizan, updated_at, created_at
      `
      )
      .eq("id", id)
      .eq("is_verified", true)
