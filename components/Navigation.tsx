...
export function Navigation() {
  ...
  // ✅ AMÉLIORATION 1 : Fonction de comptage centralisée et fiable
  const loadUnreadCount = async () => {
    if (!user || !profile) return;

    try {
      if (profile.user_type === "lawyer") {
        // Compter les messages non lus envoyés par les clients
        const { count } = await supabase
          .from("consultation_messages")
          .select("*", { count: "exact", head: true })
          .eq("is_read", false)
        ...
        // Compter les messages non lus envoyés par les avocats
        const { count } = await supabase
          .from("consultation_messages")
          .select("*", { count: "exact", head: true })
          .eq("is_read", false)
  ...
  // ✅ AMÉLIORATION 2 : Charger au montage et quand user/profile change
  useEffect(() => {
    loadUnreadCount();
  }, [user, profile]);

  // ✅ AMÉLIORATION 3 : Realtime optimisé - écoute SEULEMENT les nouveaux messages
  useEffect(() => {
    if (!user || !profile) return;
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        {
          event: "INSERT", // ← Écoute SEULEMENT les nouveaux messages
          schema: "public",
          table: "consultation_messages",
        },
        (payload) => {
          ...
          // Si c'est pas mon message et qu'il n'est pas lu
          if (newMessage.sender_id !== user.id && !newMessage.is_read) {
            // Vérifier si c'est le bon type de message
            ...
            if (shouldIncrement) {
              ...
              // ✨ NOUVEAU : Son de notification (optionnel)
              const audio = new Audio("/notification.mp3");
              audio.volume = 0.3;
              audio.play().catch(() => {}); // Ignore les erreurs si bloqué
            }
          }
        }
      )
      .on(
        "postgres_changes",
        {
          event: "UPDATE", // ← Écoute les mises à jour (quand is_read change)
          schema: "public",
          table: "consultation_messages",
        },
        (payload) => {
          ...
          // Si le message devient "lu" et ce n'est pas mon message
          if (updatedMessage.is_read && updatedMessage.sender_id !== user.id) {
            // Recharger pour avoir le bon count
            loadUnreadCount();
          }
        }
      )
  ...
  // ✅ AMÉLIORATION 4 : Demander permission notifications au premier chargement
  useEffect(() => {
