...
export function AvocatCard({ avocat, searchParams }: AvocatCardProps) {
  ...
  return (
    <Link href={getProfileUrl()} className="h-full">
      <div className="sm:hidden h-full bg-white dark:bg-[#0b1210] border border-slate-200 dark:border-[#1c2220] dark:border-[#1c2220] rounded-xl p-3 cursor-pointer transition-all duration-200 hover:border-slate-300 hover:-translate-y-1 hover:shadow-md flex flex-col items-center text-center">
        <div className="relative w-16 h-16 mb-2 flex-shrink-0">
          <div className="w-16 h-16 rounded-full overflow-hidden bg-teal-100 flex items-center justify-center font-medium text-base text-teal-700 dark:text-[#6fcf9f]">
            {avocat.avatar_url ? (
              <Image
          ...
          {isAvailableNow && (
            <span className="absolute bottom-0 end-0 w-3.5 h-3.5 rounded-full bg-teal-50 dark:bg-[#1F3D2E]0 border-2 border-white" />
          )}
        </div>
        ...
        <div className="flex items-center justify-center gap-1.5 mb-1 w-full">
          ...
          {autresSpecialites > 0 && (
            <span className="flex-shrink-0 bg-teal-100 text-teal-700 dark:text-[#6fcf9f] text-[10px] font-medium px-1.5 py-0.5 rounded">
              +{autresSpecialites}
            </span>
          )}
        </div>
      ...
      <div className="hidden sm:flex h-full bg-white dark:bg-[#0b1210] border border-slate-200 dark:border-[#1c2220] dark:border-[#1c2220] rounded-xl p-4 cursor-pointer transition-all duration-200 hover:border-slate-300 hover:-translate-y-1 hover:shadow-md flex-col">
        <div className="relative w-12 h-12 mb-3 flex-shrink-0">
          {avocat.avatar_url ? (
            ...
            <div className="w-12 h-12 rounded-full bg-teal-100 flex items-center justify-center font-medium text-base text-teal-700 dark:text-[#6fcf9f]">
              {getInitials(avocat.prenom, avocat.nom)}
            </div>
          )}
          {isAvailableNow && (
        ...
        <div className="flex items-center gap-1.5 mb-2 min-h-[20px] flex-shrink-0">
          ...
          {autresSpecialites > 0 && (
            <div className="flex-shrink-0 bg-teal-100 text-teal-700 dark:text-[#6fcf9f] text-[10px] font-medium px-1.5 py-0.5 rounded">
              +{autresSpecialites}
            </div>
          )}
        </div>
